...
export default function ChatInterface({
  ...
  return (
    <div className="w-full flex flex-col gap-6">
      ...
      {messages.length > 0 && (
        <div className="flex items-center justify-between py-2 border-b border-slate-100 bg-transparent shrink-0 select-none">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-semibold text-slate-400 hover:text-slate-700 px-3 py-1 rounded-lg hover:bg-slate-50 transition-all border border-transparent hover:border-slate-200"
          >
      ...
      {/* Thread Container */}
      {messages.length === 0 ? (
        <div className="w-full flex flex-col items-center justify-center py-16 px-6 bg-white border border-slate-100 rounded-2xl shadow-sm">
          
          <div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-4 bg-gradient-to-br from-violet-600 to-indigo-600 flex-shrink-0 shadow-md shadow-violet-100 transition-transform duration-300 hover:scale-105">
            <Sparkles className="w-6 h-6 text-white" />
          </div>

          <h3 className="text-[16px] font-black text-slate-900 mb-1.5 text-center">
            Ask Anything About These Videos
          </h3>
          <p className="text-xs text-slate-400 mb-6 text-center max-w-[340px] leading-relaxed font-semibold">
          ...
          <div className="w-full max-w-[480px] flex flex-col gap-2.5">
            {SUGGESTED_QUESTIONS.map((q) => (
              <button
                ...
                className="group w-full flex items-center justify-between gap-3 px-5 py-3.5 rounded-xl border border-slate-200/60 bg-slate-50/30 text-[12.5px] font-semibold text-slate-600 hover:border-violet-200 hover:bg-violet-50/30 hover:text-violet-700 transition-all duration-200 cursor-pointer text-left shadow-sm hover:shadow-md"
              >
            ...
            if (msg.role === "user") {
              return (
                <React.Fragment key={msg.id || index}>
                  ...
                  {isStreaming && index === messages.length - 1 && (
                    <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 select-none animate-pulse">
              ...
              return (
                <div key={msg.id || index} className="w-full animate-fade-in">
                  <div className="w-full bg-white border border-slate-200/80 rounded-2xl shadow-sm p-6 flex flex-col gap-5">
                    
                    {/* 1. Reasoning section */}
                    <div className="space-y-2">
                      <div className="flex items-center gap-1.5 text-[9px] font-extrabold text-slate-400 uppercase tracking-widest select-none">
                        <span>⚙</span> REASONING
                      </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="chat-markdown text-slate-700 text-[13px] leading-relaxed max-w-none font-semibold">
                        {msg.content ? (
                          <ReactMarkdown
                            components={{
                              p: ({ children }) => <p className="mb-3.5 last:mb-0">{children}</p>,
                              li: ({ children }) => <li className="mb-1.5 last:mb-0 list-disc ml-4">{children}</li>,
                              ul: ({ children }) => <ul className="mb-3.5 space-y-1">{children}</ul>,
                              ol: ({ children }) => <ol className="mb-3.5 list-decimal ml-4 space-y-1">{children}</ol>,
                              a: ({ children }) => (
                    ...
                    {msg.citations && msg.citations.length > 0 && (
                      <div className="rounded-xl border border-slate-200 bg-slate-50/50 p-4 space-y-3.5">
                        <div className="flex items-center gap-1.5 text-[9px] font-extrabold uppercase tracking-widest text-slate-400 pb-1.5 border-b border-slate-200/50 select-none">
                          <span>▤</span> Cited Evidence
                        </div>
                        
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                          {msg.citations.slice(0, 2).map((cit, cIdx) => (
                            <div key={cIdx} className="bg-white border border-slate-200 rounded-xl p-3.5 flex flex-col gap-3 shadow-sm">
                              
                              {/* Header quote with right-aligned timestamp pill */}
                              <div className="flex justify-between items-start gap-2.5">
                                <p className="text-xs italic text-slate-500 leading-relaxed font-semibold flex-1">
                                  &ldquo;{cit.chunk_text_preview.trim()}...&rdquo;
                                </p>
                                <span className="shrink-0 bg-slate-50 border border-slate-200/60 rounded px-1.5 py-0.5 text-[9px] font-bold text-slate-400 font-mono select-none">
                                  {getMockTimestampForChunk(cit.chunk_number)}
                                </span>
                              </div>

                              {/* Footer video labeling */}
                              <div className="text-[10px] font-extrabold text-[#4f46e5] pt-2 border-t border-slate-100 uppercase tracking-wider">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-gradient-to-t from-[#f8fafc] via-[#f8fafc]/95 to-transparent pt-10 pb-6 px-6 pointer-events-none flex justify-center">
        <div className="w-full max-w-[850px] flex flex-col items-center pointer-events-auto">
          <form onSubmit={handleSubmit} className="w-full bg-white border border-slate-200/95 rounded-2xl shadow-lg p-2.5 flex items-center gap-3.5">
            
            {/* Paperclip trigger */}
            <button
              type="button"
              onClick={handleAttachment}
              className="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-slate-50 text-slate-400 hover:text-slate-600 transition-colors cursor-pointer shrink-0"
              title="Attachment"
            >
            ...
            <input
              ...
              className="flex-1 bg-transparent outline-none text-xs text-slate-700 placeholder:text-slate-400 font-semibold"
            ...
            {isStreaming ? (
              ...
              <button
                type="submit"
                disabled={!input.trim()}
                className="w-9 h-9 flex items-center justify-center rounded-xl bg-[#4f46e5] hover:bg-indigo-700 text-white disabled:opacity-40 disabled:hover:bg-[#4f46e5] transition-all shadow-xs cursor-pointer shrink-0"
                id="chat-send"
              >
          ...
          {/* AI Accuracy Warning */}
          <p className="text-[9px] text-slate-400 mt-2.5 font-bold tracking-wide uppercase select-none">
