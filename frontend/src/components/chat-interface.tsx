...
export default function ChatInterface({
  ...
  return (
    <div className="w-full flex-1 flex flex-col min-h-0">
      ...
      {messages.length > 0 && (
        <div className="flex flex-col border-b border-slate-100 shrink-0 bg-slate-50/40">
          ...
          {messages.filter((m) => m.role === "user").length > 0 && (
            <div className="px-5 pb-3 pt-0.5 flex flex-col gap-1.5">
                ...
                {messages
                  ...
                  .map((msg, index) => (
                    <button
                      ...
                      className="text-[11px] font-medium text-slate-600 bg-white hover:bg-violet-50 hover:text-violet-700 hover:border-violet-300 border border-slate-200 rounded-lg px-2.5 py-1 transition-all duration-150 shadow-sm cursor-pointer truncate max-w-[280px]"
                      title={msg.content}
                    >
                      {msg.content}
                    </button>
